...
export const useDraftStore = defineStore("draft", () => {
  ...
  const championsByRoleForCompactView = computed(() => {
    ...
    if (frozenChampions) {
      ...
      Object.keys(grouped).forEach((role) => {
        ...
        grouped[role].scrollable.sort((a, b) =>
  ...
  function getChampionPriority(champion, role, championsStore) {
    ...
    if (
      isBannedChampion(champion.name) ||
      unavailableChampions.value.has(champion.name)
    )
      return 4;
  ...
  async function debugPrintLcuDrafts() {
    ...
    const lcuDrafts = await fetchLcuDraftsFromFirestore(
    ...
    lcuDrafts.forEach((draft, index) => {
      console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
      console.log(
        `Draft #${index + 1} (Lobby: ${draft.lobbyId || "N/A"}, Phase: ${draft.phase})`,
      );
      console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
  ...
  function togglePick(championName, pickedFromRole = null) {
    ...
    if (index === -1) {
      ...
      const contextRole =
        pickedFromRole || champion.mainRole || champion.roles?.[0];
      ...
      pickContext.value = pickContext.value.filter(
        (ctx) => ctx.championName !== championName,
      );
  ...
  function placeChampionInPanel(champion, preferredRole = null) {
    ...
    if (
      preferredRole &&
      validRoles.has(preferredRole) &&
      unavailablePanelState.value[preferredRole]
    ) {
    ...
    const championRoles = [
      champion.mainRole,
      ...(champion.roles || []).filter((r) => r !== champion.mainRole),
    ];
    const validChampionRoles = championRoles.filter((role) =>
      validRoles.has(role),
    );
  ...
  function reconstructPickContext() {
    ...
    draftSeries.value.forEach((championName, index) => {
      ...
      for (const role of ["top", "jungle", "middle", "bottom", "support"]) {
        if (
          unavailablePanelState.value[role] &&
          unavailablePanelState.value[role].includes(championName)
        ) {
          foundRole = role;
          break;
        }
      ...
      if (!foundRole) {
        foundRole =
          champion.mainRole &&
          ["top", "jungle", "middle", "bottom", "support"].includes(
            champion.mainRole,
          )
            ? champion.mainRole
            : champion.roles &&
