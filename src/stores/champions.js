...
export const useChampionsStore = defineStore('champions', () => {
  ...
  function getRoleIconUrl(role) {
    const svgContent = getRoleIconSvg(role)
    if (svgContent.startsWith('<div')) {
      // Return a data URL for the unknown placeholder
      return `data:image/svg+xml;base64,${btoa('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-[14px] h-[14px]"><rect width="24" height="24" fill="#d1d5db" rx="2"/></svg>')}`
    }
